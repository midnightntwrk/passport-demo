...
export function createColourPayer(deps: {
  ...
  const payInto = async (address: string): Promise<ColourPayment> => {
    ...
    /* A GIFT INTO A k1-ARM ACCOUNT IS REFUSED, before a coin is minted.
       Every gift is shielded, and `deposit_shielded` on the reference contract
    ...
    if (!accountDeposits(module).mirrorsShieldedBalance) {
      throw new ColourPayFailure(
        501,
        'k1-inbox-entry-required',
        `${name} cannot be paid into ${address}: it is a k1-arm account, and deposit_shielded there takes a 192-byte InboxEntry v1 alongside the coin that this service cannot build. Nothing was minted and nothing was spent.`,
      );
    }
