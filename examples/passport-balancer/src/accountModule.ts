 ...
 *
 * A THIRD build joined the two on 2026/09/16: `account-custody`, the compiler
 * output of the account custody contract, which is a different contract rather
 * than a later revision of this one — a different circuit set, a different
 * vocabulary for the same deposits, a different ledger shape, and ZKIR v3 where
 ...
 *
 * The custody question is asked FIRST because it is the decisive one: the
 * account custody contract has no `transfer_shielded_to_account` either, so a
...
export interface AccountDeposits {
   ...
   * account custody contract pairs the coin claim with an inbox entry, and
   ...
   * confirmed by somebody who was not the depositor. The account custody
   * contract is STATELESS shielded custody by design (MIP-0012 §6.1): no held
   * coin's
 ...
 *
 * The account custody build is compiled `--feature-zkir-v3`, and neither of those can
 ...
 * how a custody deposit would be sent to a server that cannot answer it.
 */
export function proverForModule(
  ...
  if (config.provingServerUrlV3) {
    return {
      ...
      why: `account-custody is ZKIR v3 and proves at ${V3_PROVER_ENV}`,
    };
  }
  return {
    kind: 'refused',
