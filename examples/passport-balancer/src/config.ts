...
export interface BalancerNetworkEndpoints {
  ...
  /**
   * A proof server that can prove ZKIR v3, when one exists — the `account-k1`
   ...
   *
   * There is deliberately no default: unset, the k1 path refuses in one line
...
export interface BalancerConfig extends BalancerNetworkEndpoints {
  ...
  /**
   * Overrides the search for the compiled k1-arm reference build's ZK
   * artefacts. `BALANCER_ACCOUNT_K1_ASSETS`.
   *
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): BalancerConfig {
  ...
  return {
    ...
    ...(trimmed(env.BALANCER_ACCOUNT_ASSETS)
      ...
      : {}),
    ...(trimmed(env.BALANCER_ACCOUNT_K1_ASSETS)
      ? { accountK1AssetsPath: trimmed(env.BALANCER_ACCOUNT_K1_ASSETS) as string }
      : {}),
