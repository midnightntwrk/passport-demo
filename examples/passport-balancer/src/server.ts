...
import { PROVE_ACCOUNT_CUSTODY_PATH, PROVE_ACCOUNT_CUSTODY_PREFIX, custodyProverFromConfig } from './proveAccountCustody.js';
...
async function main(): Promise<void> {
  ...
  const describeRate = (limit: { perMinute: number; burst: number }): string =>
    limit.perMinute > 0
      ...
      : 'no per-client limit';
  console.log(
    `limits    /balance-only ${describeRate(config.balanceRate)}; /register-alias ${describeRate(config.aliasRate)}; /fund-account ${describeRate(config.accountRate)}; /prove-account-custody ${describeRate(config.proveAccountCustodyRate)}`,
  );
  console.log(
  ...
  /* `/prove-account-custody` spends nothing, so it is not on any of the three above: a
     caller that has used its grant allowance for the hour must still be able to
     finish proving the Passport that allowance opened. See `./proveAccountCustody.ts`. */
  const proveAccountCustodyBucket = new TokenBucket({
    ratePerMinute: config.proveAccountCustodyRate.perMinute,
    burst: config.proveAccountCustodyRate.burst,
  });
  const spendAdmission = new SpendAdmission(config.spendQueueMax);

  /**
   * The account custody proving route.
   *
  ...
  try {
    ...
    console.log(`[account] ${accountFunder.custodyReadiness}`);
  ...
  const status = async (): Promise<Record<string, unknown>> => {
    ...
    return {
      ...
      /* `/prove-account-custody`'s own book, and nothing of the sponsor's. `configured`
         ...
         internet. See `./proveAccountCustody.ts`. */
      accountCustodyProving: custodyProver.snapshot(),
      ...
      limits: {
        ...
        clientsTracked:
          balanceBucket.size + aliasBucket.size + accountBucket.size + proveAccountCustodyBucket.size,
  ...
  const giftDesk: GiftDesk = createGiftDesk({
    ...
    /* The third request shape. An account contract is paid by a circuit the
   ...
   *
   * Split out for `/prove-account-custody`, which meters a caller exactly like a spend route
  ...
  const server = createServer((request, response) => {
    ...
    void (async () => {
      ...
      /* METERED BUT NOT ADMITTED. `/prove-account-custody` gets the same per-client bucket
         ...
         at a time, bounded inside `./proveAccountCustody.ts`. */
      if (request.method === 'POST' && path === PROVE_ACCOUNT_CUSTODY_PATH) {
        const proveGuard = { prefix: PROVE_ACCOUNT_CUSTODY_PREFIX, bucket: proveAccountCustodyBucket };
        ...
        try {
          ...
          console.warn(
            `[${PROVE_ACCOUNT_CUSTODY_PREFIX}] refused: invalid-request — ${cause instanceof Error ? cause.message : String(cause)}`,
          );
        ...
        /* THE CALLER'S CONNECTION, handed to the queue. A account custody proof is a minute
      ...
      if (request.method === 'POST' && path === '/gift-nft') {
        ...
        try {
          ...
          respond(request, response, 400, {
            error: 'invalid-request',
            message: `The request body must be JSON. ${GIFT_REQUEST_SHAPES}`,
          });
          return;
        }
        /* Every other judgement — which of the three shapes this is, whether
           the name resolves, whether the address is ours — belongs to the
           desk, and `docs/demo/partner-api.md` is written from it. */
        const outcome = await giftDesk.give((body ?? {}) as GiftRequestBody);
        respond(request, response, outcome.status, outcome.body);
        return;
      }
