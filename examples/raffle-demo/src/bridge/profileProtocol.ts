 ...
 *
 * Source: midnight-passport-dynamic-signing, packages/connect/src/protocol/profile.ts
 * Vendored: 2026/09/02
 *
 ...
 * what `passportContract.address` carries. The transaction engine's own
 * addresses are a signing detail no dApp has a legitimate use for, and an app
 * that paid one of them would have paid somewhere the account cannot spend
 * from — so they are not offerable, and a request naming one is answered
 * `invalid_request` rather than dropped in silence.
 *
 * ---------------------------------------------------------------------------
