...
/**
 * The passkey pointer store and the routing rule, re-exported from
 * `../lib/custodyRoute.ts`.
 *
 * THEY LIVE THERE AND NOT HERE FOR ONE MEASURED REASON. `App.tsx` asks the
 * routing question on every render, and importing it from this module would
 * pull this module's record format, its name store, and its stage machine into
 * the entry chunk — 29,701 bytes of it, the same measurement that moved
 * `choosePassportIdentity` out to `../lib/dynamicSession.ts` on 2026/09/16.
 * `custodyRoute.ts` imports nothing at run time. They are re-exported here so a
 * reader of the custody stores finds them where they expect to.
 */
export {
  CUSTODY_PASSKEY_KEY,
  custodyPasskeyKey,
