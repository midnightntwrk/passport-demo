 ...
 *
 * `v` is 2 since 2026/09/26, and this build reads 1 and 2. Format 2 is what
 * release v4.0 wrote for the few hours this file carried one key — the viewing
 * key of an account custody Passport, in a `viewingKeys` container beside the
 * three record sets (#108). That key travels with the way back now
 * (`./signInViewingKeys.ts`), so this build writes format 2 with no such
 * container, and a v4.0 file that has one still restores: the payload is
 * projected onto the three record sets before anything reads it, so the key in
 * it is never read, kept, or applied. The cipher, the KDF, and the header bytes
 * are the same in both formats, so either is opened by the same code.
 *
 ...
 *
 * 2 since 2026/09/26 — see the header on what format 2 was, and why a file of
 * it written by release v4.0 still restores.
 */
 ...
 *
 * Format 1 is every file written before 2026/09/26, and a file somebody kept is
 * a file they expect to restore — so it is read, as it always was.
 */
...
export async function openPassportBackup(
  ...
  const contents: PassportBackupContents = {
    ...
    /* AND NOTHING ELSE. A format-2 file from release v4.0 may carry a
       `viewingKeys` container beside these three; it is left behind here,
       unread — see the header. */
  };
