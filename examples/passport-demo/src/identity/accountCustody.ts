 ...
 *
 * THREE BUILDS SINCE 2026/09/17. `withdraw_shielded_with_k256` is asked about
 ...
 * ABSENCE of `transfer_shielded_to_account`, and the account custody build —
 * which shares no circuit name with either — is absent it too, so the older
 * question alone would read a custody account as `account-v1` and hand it a
 * module that cannot open it. Nothing deploys one yet, so in practice every
 * account on the chain still answers `account` or `account-v1`, exactly as it
 * did.
 *
