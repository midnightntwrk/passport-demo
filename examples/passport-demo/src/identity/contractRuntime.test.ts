 ...
 * in-tab worker when nothing is named. `account-k1` is ZKIR v3, which neither
 ...
 * unsupported IR. So k1 reads its own list and an empty one is a refusal.
 */
describe('contractProvingRoute', () => {
  ...
  it('sends k1 to the v3 list, and to nothing else', () => {
  ...
  it('never lets k1 fall back to the in-tab prover', () => {
    expect(
      contractProvingRoute({ provingServerUrls: V2, provingServerUrlsV3: V3 }, 'account-k1')
        .mayProveInTab,
    ).toBe(false);
  });
  ...
  it('refuses in plain words when no v3 server is configured', () => {
    ...
    expect(() =>
      contractProvingRoute({ provingServerUrls: V2, provingServerUrlsV3: [] }, 'account-k1'),
  ...
  it('tells the OPERATOR which variable is unset, on the console rather than the screen', () => {
    ...
    expect(() =>
      contractProvingRoute({ provingServerUrls: [], provingServerUrlsV3: [] }, 'account-k1'),
    ).toThrow();
