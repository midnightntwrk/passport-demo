 ...
 * ------------------------------
 * The same split `accountK1Plan.ts` and `accountK1Custody.ts` already keep: the
 ...
 * piece of work (`docs/demo/k1-custody-layer-design.md` §3). Until that lands a
 ...
 *
 * Both prototype builds carry `deposit_night`; the k1 build renamed it
...
export function depositCircuitFor(module: PassportContractName): DepositCircuit | null {
  ...
  if (module === 'account-k1') return 'deposit_unshielded';
  return null;
}
...
export interface K1WithdrawLeg {
  /** Unsuffixed, the way `K1CallRequest.operation` wants it. */
...
export interface K1SendPlan {
  readonly withdraw: K1WithdrawLeg;
  readonly deposit: K1DepositLeg;
}

/** Everything a plan is built from. */
export interface K1SendPlanInput {
 ...
 */
export function k1SendRefusal(input: K1SendPlanInput): string | null {
 ...
 * asked {@link k1SendRefusal} — the throw is the assertion that it did, and it
 * carries the same sentence so a caller that forgot still shows something a
 * person can read.
 */
export function planK1Send(input: K1SendPlanInput): K1SendPlan {
  const refusal = k1SendRefusal(input);
  if (refusal !== null) throw new Error(refusal);
  /* NARROWING, AND NOT A FALLBACK. `k1SendRefusal` above has already refused
 ...
 */
export function k1ApprovalPrompt(provider: string | null): string {
...
export const K1_APPROVAL_WAITING = 'Waiting for your approval';
...
/* -------------------------------------------------------------------------- */
/* The account's own NIGHT, read off the k1 ledger                            */
/* -------------------------------------------------------------------------- */

/**
 * The mirrored unshielded map, as the compiled `account-k1` build exposes it.
 *
 * `K1Ledger` in `accountK1Custody.ts` carries only what a gated call needs —
 ...
 */
export interface K1UnshieldedLedger {
 ...
 * have.
 */
export function k1UnshieldedBalance(
  ledger: K1UnshieldedLedger,
  colour: Uint8Array,
): bigint {
