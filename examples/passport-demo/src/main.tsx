...
if (!import.meta.env.DEV || window.location.origin === requiredDevelopmentOrigin) {
  ...
  if (import.meta.env.VITE_DYNAMIC_ENVIRONMENT_ID) {
    ...
    /* The Dynamic Passport milestone, when somebody has asked for it — a build
       made for it (`VITE_K1_MILESTONE=1`) or a `?k1=1` on one that was not.
    ...
    if (
      import.meta.env.VITE_K1_MILESTONE === '1' ||
      new URLSearchParams(window.location.search).get('k1') === '1'
    ) {
      void Promise.all([import('./screens/K1Milestone.js'), import('react-dom/client')])
