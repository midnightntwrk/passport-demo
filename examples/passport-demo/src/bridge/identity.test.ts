...
/**
 * A config whose MPC key is the secp256k1 generator point G (compressed) — a
 * valid, well-known point used ONLY to make this derivation deterministic. It is
 * not the live vault's MPC key, so this address is a test fixture, not a real
 * deposit address.
 */
const TEST_CONFIG: BridgeConfig = {
  ...
  mpcSecp256k1Pubkey: '0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798',
...
describe('signet vault identity', () => {
  ...
  it('derives a deterministic Sepolia deposit address from the commitment', () => {
    expect(signetDepositAddress(TEST_SECRET, TEST_CONFIG)).toBe(
      '0xB3fd9E5801E40B2C79144270f43bccc216D6bdB9',
    );
  });
});
