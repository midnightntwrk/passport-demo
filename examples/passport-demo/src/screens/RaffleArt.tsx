 ...
 * make, and no third-party mark anywhere in it. Every colour is a
 * `tokens.css` custom property via `currentColor` and explicit `var(…)`
 * references, so the drawing follows the theme instead of fighting it — a
 * flat PNG would be legible in exactly one of the two.
 *
 * `title` is rendered only when the art carries meaning on its own. On a card
 * that already names the app it is decorative, so the default is a silent
 * `aria-hidden` graphic.
 */
