...
import { K256_ENVELOPE_NONE, k256Challenges, type K256DeviceIdentity } from '../identity/accountK1.js'
import {
  activateK1Device,
  defaultK1Deps,
  deployK1Account,
...
} from '../identity/accountK1Session.js'
import {
  k1ApprovalPrompt,
  k1SendRefusal,
  k1UnshieldedBalance,
...
} from '../identity/accountK1Send.js'
...
export default function DynamicPassport({ network }: DynamicPassportProps) {
  ...
  const createPassport = useCallback(() => {
    ...
    void run(PHASE_LABELS.deploy, async () => {
      ...
      let record = (await deployK1Account(k1Session(session), identity, onPhase)).record
      if (!record.activated) {
        record = (await activateK1Device(k1Session(session), identity, onPhase)).record
      }
      refresh()
    })
    ...
    (alias: string) => {
      void run('Choosing your name', async () => {
        ...
        /* The name's owner secret comes from this device's own transaction key
           ...
           */
        const { k1WalletSeed } = await import('../identity/accountK1Custody.js')
        const ownerKey = await deriveMidnamesOwnerKey(k1WalletSeed(deps, user))
        ...
        saveK1Name(window.localStorage, user, network, alias)
        refresh()
      })
    },
  ...
  const readBalance = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const ledger = contractModule.ledger(state.data) as unknown as K1UnshieldedLedger
      setBalance(k1UnshieldedBalance(ledger, nightColourBytes()))
    } catch (cause) {
      console.warn('[k1] could not read this Passport', cause)
  ...
  const findByName = useCallback(
    async (typed: string): Promise<NameRecoveryOutcome> => {
      ...
      try {
        ...
        console.warn('[k1] could not check that name', cause)
      }

      const outcome = k1RecoveryOutcome(resolved, { operations, holdsDevice })
      if (outcome.kind !== 'found' || pk === null) return outcome
      const restored: K1AccountRecord = recoveredK1Record({
        user,
        network,
        address,
      ...
      saveK1Record(window.localStorage, restored)
      saveK1Name(window.localStorage, user, network, label)
      refresh()
      return outcome
    },
    ...
    <Shell label="Passport">
      ...
      <div className="mndyn-actions">
        ...
        <p className="mnob-hint" role="status">
          {busy !== null || (view?.record && nextK1Step(view.record) !== 'ready')
