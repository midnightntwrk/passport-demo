...
/**
 * PAYING A RAW ADDRESS IS LEGS NOW, AND THE REVIEW SAYS SO (2026/09/17).
 *
 * THE DEFECT THIS UNIT EXISTS TO PREVENT. A shielded send to an
 * `mn_shield-addr…` used to be one transaction: a PARTIAL withdrawal straight
 * out of the sender's account. That is the branch of the deployed contract that
 * splits its coin and re-registers the remainder, and the network refuses every
 * later withdrawal against what that leaves behind — so the first such send
 * worked and every send after it, for the life of the Passport, did not. Otrix
 * reproduced it on several accounts.
 *
 * The send now takes the same legs a name send takes: the whole coin out, the
 * amount paid to the address, the change back afterwards. What a browser can
 * answer, and this tier cannot get past, is whether the person is TOLD that
 * before they confirm — a sheet that still promised one transaction over three
 * would be describing the send that caused the defect.
 */
test('a shielded send to an address is reviewed as the legs it now takes', async () => {
  await chooseShielded();
  await recipient().fill('');
  await recipient().fill(SHIELDED);
  await expect(refusal()).toHaveCount(0);

  /* Less than the account holds, which is what makes it a three-leg send: the
     whole coin comes out and the remainder goes back. */
  await page.getByPlaceholder('0', { exact: true }).fill('1');
  await page.getByRole('button', { name: /^Review$/ }).click();

  const review = await page.locator('.mnhome-send-rows').innerText();
  expect(review).toContain('1 mUSD');
  /* THE ROW THAT DID NOT EXIST FOR AN ADDRESS UNTIL THIS DATE. Matched
     case-insensitively: the term's own heading is upper-cased in CSS. */
  expect(review).toMatch(/how it goes/i);
  expect(review).toContain('Two steps');
  /* The three-leg sentence, in the address's own words: it never calls the
     recipient "them" and never says the amount goes into an account, because a
     raw address has neither. */
  expect(review).toContain('then the amount goes to that address');
  expect(review).toContain('Your change comes back to you on its own');
  expect(review).not.toContain('into theirs');
  /* And it does not compare this send to itself. The name path's sentence ends
     "so this takes longer than sending to an address", which over a send TO an
     address is a comparison with nothing on the other side of it. */
  expect(review).not.toContain('takes longer than sending to an address');
  /* NOT ONE TRANSACTION. The one-transaction circuit hands an amount to a
     CONTRACT address; there is no contract behind a pasted address, so a sheet
     that said "Transferring" here would be promising a transaction that cannot
     be built. */
  expect(review).not.toContain('Transferring');
  expect(review).not.toContain('in one network transaction');
  /* The rules every other review in this file holds to: no colour, no account
     of ours, and no machinery on the step that confirms. */
  expect(review).not.toContain(PASSPORT_ACCOUNT_ADDRESS);
  expect(review).not.toMatch(/registry|indexer|resolver|contract|circuit|DUST/i);

  await page.getByRole('button', { name: /^Back$/ }).click();
  /* Back to the pair the next test starts from. This file runs serial against
     one open sheet, and leaving mUSD and a pasted address behind would change
     what the test below reviews. */
  await picker().selectOption('night');
  await recipient().fill('');
  await recipient().fill(`${RESOLVABLE_NAME}.night`);
