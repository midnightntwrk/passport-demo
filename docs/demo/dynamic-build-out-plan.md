...
**PR sequence:** (1) k1 artefacts, module loader, HTTP-only proving route; (2) a private coin store that survives reloads; (3) the k1 custody module — deploy in waves, activate with the Dynamic key, deposit, withdraw; (4) the sponsor recognises the third build for the opening balance and gifts; (5) Welcome and Home: the Dynamic path end to end, and recovery by name; (6) live walks on staging with a Dynamic login, then the release. Design and skeleton for (3): PR #55.
