 ...
 *
 *   --require-account-custody
 *                        refuse to cut the release unless the
 *                        `account-custody` artefacts are in this tree. They
 *                        are gitignored, so an ordinary checkout has none and
 *                        the bundle is simply cut without them; pass this for
 *                        the release that ships the Dynamic-only Passport,
 *                        which cannot work without them.
 *
 ...
 *   PASSPORT_RELEASE_REQUIRE_ACCOUNT_CUSTODY
 *                            `1` is the same as `--require-account-custody`.
...
/**
 * The account custody build's, WHEN THE TREE HAS THEM — the same view
 * `prepare-zk-assets.mjs` takes of the same directories.
 *
 ...
 * absence only when the release is asked to guarantee them — `--require-account-custody`, or
 * `PASSPORT_RELEASE_REQUIRE_ACCOUNT_CUSTODY=1`, which is what the release shipping the
 ...
 * is the one thing that is fatal whether or not that build was asked for.
 */
const optionalZkDirectories = [
  'examples/passport-balancer/contracts-stagenet/managed/account-custody/keys',
  'examples/passport-balancer/contracts-stagenet/managed/account-custody/zkir',
];
...
/* The release that ships the Dynamic-only Passport says so, and then a tree
   without the account custody artefacts is an error rather than a smaller
   bundle. */
const requireAccountCustody =
  process.argv.includes('--require-account-custody') || process.env.PASSPORT_RELEASE_REQUIRE_ACCOUNT_CUSTODY === '1';
 ...
 * actually has — will this release carry account-custody? — instead of discovering
...
function zkDirectoriesToPack() {
  ...
  if (presentOptional.length > 0 && presentOptional.length !== optionalZkDirectories.length) {
    fail(
      'the account-custody build is half here — ' +
  ...
  const carriesAccountCustody = presentOptional.length === optionalZkDirectories.length;
  if (!carriesAccountCustody) {
    if (requireAccountCustody) {
      fail(
        'this release was asked to carry the account-custody artefacts and they are not in this tree. ' +
          'Run `node scripts/fetch-zk-artefacts.mjs`, or cut it from the host that compiled the build.',
      );
    }
    console.log(
      'tag-release: the account-custody artefacts are not in this tree, so the bundle will not carry ' +
        'them; everything else is packed as usual. Pass --require-account-custody to make this fatal.',
    );
  }
  return carriesAccountCustody ? [...requiredZkDirectories, ...optionalZkDirectories] : requiredZkDirectories;
}
...
/* Decided here, in the read-only preflight, so `--dry-run` says what the bundle
   would carry and `--require-account-custody` refuses before anything is created. */
